...
function NavBar({ scroll }: { scroll: number }) {
    return (
        <div className={`nav-bar z-20 w-full flex items-center h-8 text-md text-xl text-white gap-2 px-5 bg-smoky-black ${scroll > 650 ? 'bg-opacity-100 fixed top-0' : 'bg-opacity-0'}`}>
            {["About", "Events", "Competitions", "Insights"].map((item, index) => {
                if (index != 0) {
                    return (
                        <>
                        <div className='w-1 h-1 mt-1 bg-white rounded-full' />
                        <div className=' flex items-center hover:text-muave-pink hover:underline text-white transition-colors'>{item}</div>
                        </>
                    )
                } else {
                    return (
        ...
        <div id="splash" className={`absolute z-10 top-0 w-screen h-screen flex flex-col justify-center`}>
            ...
            <div className='my-2 px-3 z-10'>
                <span className='flex items-center gap-[35px] ml-[10px] flex-wrap'>
                    ...
                    />
                    <h1 className='flex items-center text-8xl text-white font-bold leading-none'>CS CLUB</h1>
