...
import { FC, useState, useContext , useEffect , use } from "react";
...
const testAnnouncements: AnnouncementListing[] = [
    ...
    },
    {
        ...
        message: {
            ...
            "ja": "こんにちは、さいきんのLeetCodeコンテストにさんかしたすべてのひとにかんしゃしたいとおもいます! 次回は、大学院生と学部生の両方の競争を同時に実施する予定です。",
        },
        href: "https://www.google.com"
    },
];
...
const Announcement: FC<APIMessage> = ({...props}) => {
    ...
    return (
        <div className="max-w-full h-auto min-h-[100px] max-h-[300px] flex flex-col-reverse lg:flex-row mb-[25px] gap-[3px] text-base bg-transparent relative z-20">
            <div className="group relative flex-grow max-w-[600px] h-auto w-auto flex flex-col gap-3 mt-[10px] items-center overflow-x-clip pr-[10px]">
                ...
                <span className="flex-grow h-auto font-normal text-justify text-sm sm:text-base md:text-xl text-secondary-200 leading-tight transition-all px-[10px] py-[20px] group-hover:rotate-[-3deg]">{ props.content }</span>
            ...
            <div className="h-auto flex flex-col text-secondary-200 gap-1">
                ...
                <div className="flex flex-row h-[12px] items-center gap-2 pl-[1px] mt-[40px] mb-1">
                    <img className="h-full w-auto opacity-[30%]" src="/discord_logo.png" alt={{en: "Discord", ja: "ディスコード", es: "Discord"}[lang]} />
                    <GenericTimestamp className="text-secondary-700 font-light text-xs" date={props.timestamp} time={props.timestamp} />
                </div>
                { props.channel_id && 
                <h2 className="flex flex-row items-center gap-2 mb-1">
                    ...
                    <GenericTimestamp className="text-secondary-700 font-light text-xs" date={"Announcements"} />
                </h2> }
                <h2  className={`text-2xl font-semibold  w-full leading-none`}>@{props.author.username}</h2>
                <div className="w-full h-auto flex mt-[2px]">
                    <StyledAnchor onMouseEnter={() => setIsLinkHover(true)} onMouseLeave={() => setIsLinkHover(false)} href={`https://discord.com/channels/925594432068280331/${props.channel_id}/${props.id}`} icon={ButtonIcon}>
                    {{
                        "en": "View Post",
...
const fetchAnnouncements: Promise<any> = fetch("/api/announcements").then( (res) => res.json() );

export default function Announcements() {
    const [isSelect, setIsSelect] = useState<boolean>(false);
    const { lang } = useContext(LangContext);
    const announcements = use(fetchAnnouncements) as APIMessage[];
    ...
    <div className="w-full h-auto max-h-screen max-w-[calc(100vw_-_40px)] overflow-y-scroll flex flex-col gap-x-4 relative text-4xl text-secondary-200 font-bold mt-[25px]">
        ...
        { announcements.filter((announcement: APIMessage) => announcement.author.username !== "MEE6").map( (announcement: APIMessage, index: number) => <Announcement key={"EventDeckCard-" + index} {...announcement} /> ) }
        </TimelineMini>
    </div>
